		...
		function directorypress_comments( $comment, $args, $depth ) {
			...
			if(have_comments()){
				...
				echo '<li '. esc_attr($comment_class) .' id="li-comment-'. esc_attr($comment->comment_ID) .'">';
					echo '<div class="directorypress-single-comment '. esc_attr($userClass) .' clearfix" id="comment-'. esc_attr($comment->comment_ID) .'">';
						echo '<div class="comment-content">';
							...
							if ( !empty( $dirrater_title )  ) {
								echo '<p class="dirrater_title">' . esc_html($dirrater_title) . '</p>';
							}
							if ( ! empty( $rating ) ) {
								echo '<div class="review_rate" data-dirrater="' . esc_html($rating) . '"></div>';
							}
						...
						echo '<div class="comment-meta-main clearfix">';
							echo '<div class="author-img">';
								...
								if(!empty($avatar_id) && is_numeric($avatar_id)) {
									...
									echo '<img src="' . esc_url(bfi_thumb( $image_src_array, $params )) . '"  alt="'. esc_attr(get_the_author_meta('display_name', $comment->user_id)).'" />';
								}else{
							...
							echo '<div class="comment-meta">';
								...
								echo '<time class="comment-time">'. esc_html(get_comment_date()) .', '. esc_html(get_comment_time()) .'</time>';
	...
	echo '<section id="comments">';
		...
		if ( have_comments() ){ 
			...
			echo '<div class="single-post-fancy-title comments-heading-label">';
				echo '<span>'. esc_html($comments_label) .' <span class="comments_numbers">'. esc_html(number_format_i18n( get_comments_number() )) .'</span></span>';
			echo '</div>';
