...
<div id="directorypress-images-upload-wrapper" class="gallary_images">
	<p class="directorypress-submit-field-title"><?php esc_html_e('Listing images', 'DIRECTORYPRESS'); ?></p>
	...
	<div id="directorypress-attached-images-wrapper" class="gallery clearfix">
		...
		<div class="directorypress-attached-item gallary_images">
			...
			<div class="thumb-links clearfix">
				<div class="directorypress-attached-item-logo directorypress-radio checkbox">
					<label title="<?php esc_html_e("Set as Thumbnail Image", "DIRECTORYPRESS"); ?>">
				...
				<div class="directorypress-attached-item-delete gallary_images dicode-material-icons dicode-material-icons-trash-can-outline" title="<?php esc_html_e("delete", "DIRECTORYPRESS"); ?>"></div>
			</div>
		</div>
		<?php endforeach; ?>
		<?php if(is_admin()){ ?>
			...
			<div class="directorypress-drop-attached-item gallary_images">
				<div class="directorypress-drop-zone gallary_images">
					<div class="dropzone-content">
						...
						<span class="drophere"><?php esc_html_e("Drag and Drop Images Here", "DIRECTORYPRESS"); ?></span>
						<span class="drophere-desc-text"><?php esc_html_e("Photos must be JPEG or PNG format with ideal Aspect Ratio 16:9 ", "DIRECTORYPRESS"); ?></span>
						<button class="directorypress-upload-item-button gallary_images btn btn-primary"><?php esc_html_e("Browse Files", "DIRECTORYPRESS"); ?></button>
		...
		(function($) {
			...
			$(function() {
				$('#directorypress-admin-upload-image').click(function(event) {
					...
					var frame = wp.media({
						title : '<?php echo esc_js(sprintf(esc_html__('Upload image (%d maximum)', 'DIRECTORYPRESS'), $listing->package->images_allowed)); ?>',
						multiple : true,
						library : { type : 'image'},
						button : { text : '<?php echo esc_js(esc_html__('Insert', 'DIRECTORYPRESS')); ?>'},
					});
								...
								if (images_allowed_gallary_images > $("#directorypress-attached-images-wrapper.gallery .directorypress-attached-item.gallary_images").length) {
									...
									$.ajax({
										...
										data: {
											...
											'_wpnonce': '<?php echo esc_js(wp_create_nonce('upload_images')); ?>',
										},
										attachment_id: attachment.id,
