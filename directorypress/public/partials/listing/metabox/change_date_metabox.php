	...
	"use strict";
		...
		$(function() {
			$("#expiration_date").datepicker({
				...
				dateFormat: '<?php echo wp_kses_post($dateformat); ?>',
			...
			<?php
			if ($lang_code = directorypress_dplc(get_locale())): ?>
			$("#expiration_date").datepicker($.datepicker.regional[ "<?php echo esc_attr($lang_code); ?>" ]);
			<?php endif; ?>
	
			$("#expiration_date").datepicker('setDate', $.datepicker.parseDate('dd/mm/yy', '<?php echo esc_attr(date('d/m/Y', intval($listing->expiration_date))); ?>'));
		});
	})(jQuery);
</script>
<?php global $DIRECTORYPRESS_ADIMN_SETTINGS; ?>
<?php if (!$DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_change_expiration_date'] && current_user_can('manage_options')): ?>
<p><?php esc_html_e("Regular users can not change expiration date. This option is available only for admins.", 'DIRECTORYPRESS'); ?></p>
<?php endif; ?>

<p><?php esc_html_e("Set new expiration date and time of the listing.<br />Be careful: If you'll set past date - listing will expire in some minutes.", 'DIRECTORYPRESS'); ?></p>

<div class="directorypress-content-wrap-fields-metabox">
	<div class="row directorypress-field directorypress-form-group directorypress-form-horizontal">
		
		<div class="col-lg-4 col-md-4 col-sm-12">
			<label class="directorypress-col-md-1 directorypress-control-label">
				<?php esc_html_e('Date:', 'DIRECTORYPRESS'); ?>
			</label>
		...
		<div class="col-lg-4 col-md-4 col-sm-12">
			...
			<label class="directorypress-col-md-1 directorypress-control-label">
				<?php esc_html_e('Hour:', 'DIRECTORYPRESS'); ?>
			</label>
			<select name="expiration_date_hour" class="form-control <?php if(!is_admin()){ echo 'directorypress-select2'; } ?>">
		...
		<div class="col-lg-4 col-md-4 col-sm-12">
			<label class="directorypress-col-md-1 directorypress-control-label">
				<?php esc_html_e('Minutes:', 'DIRECTORYPRESS'); ?>
			</label>
			<select name="expiration_date_minute" class="form-control <?php if(!is_admin()){ echo 'directorypress-select2'; } ?>">
...
<?php do_action('directorypress_changedate_html', $listing); ?>