		...
		<div id="main" class="wrapper">
			...
			<?php while ($public_handler->query->have_posts()): ?>
				...
				<div class="directorypress-listing-text-content-wrap entry-content">
					...
					<em class="directorypress-listing-date"><?php echo get_the_date(); ?> <?php echo esc_html(get_the_time()); ?></em>
				...
				<?php if (directorypress_has_map()): ?>
					...
					<h2><?php esc_html_e('Map', 'DIRECTORYPRESS'); ?></h2>
					<?php $listing->display_map($public_handler->hash, false, true); ?>
					<?php endif; ?>
				<?php endif; ?>
				
				<?php if (count($listing->images) > 1): ?>
				<h2><?php esc_html_e('Images', 'DIRECTORYPRESS'); ?> (<?php echo count($listing->images); ?>)</h2>
				...
				<h2 class="comments-title">
					<?php
						printf(esc_html(_n('One thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;', get_comments_number(), 'DIRECTORYPRESS')),
							esc_attr(number_format_i18n(get_comments_number())), '<span>' . esc_attr(get_the_title()) . '</span>');
					?>
				</h2>
