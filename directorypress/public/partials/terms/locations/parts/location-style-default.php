<?php
	...
	echo '<div id="loaction-styles'. esc_attr($instance->args['id']).'" class="row location-style-'. esc_attr($instance->location_style) .' grid-item directorypress-locations-columns clearfix">';
		...
		foreach ($terms AS $key=>$term) {
			...
			echo '<div class="directorypress-location-item col-lg-' . esc_attr($instance->col) . ' col-md-' . esc_attr($instance->col_tab) . ' col-sm-' . esc_attr($instance->col_mobile) . '">';	
				echo '<div class="directorypress-location-item-holder">';
					echo '<div class="directorypress-parent-location">';
						echo '<a href="' . esc_url(get_term_link($term)) . '" title="' . esc_attr($term->name) . esc_attr($term_count) . '"><span class="location-icon"><i class="dicode-material-icons dicode-material-icons-map-marker-outline"></i></span><span class="loaction-name">' . esc_html($term->name) .'</span><span class="location-item-numbers">'. esc_html($term_count) .'</span></a>';
					echo '</div>';
					if($instance->depth > 1){
