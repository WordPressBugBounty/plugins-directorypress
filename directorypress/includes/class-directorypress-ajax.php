...
class directorypress_ajax {
	...
	public function directorypress_handler_demand() {
		...
		if ( ! wp_verify_nonce( $_POST['dp_ajax_nonce'], 'directorypress-ajax-nonce' ) ) {
			 die ( 'No Kiddies!');
		}
			...
			case "directorypress_listings_handler":
				if ($_POST['handler'] == "directorypress_directory_handler"){
					$shortcode_atts = array_merge(array(
							...
							'hide_order' => (!($DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_show_orderby_links'])),
							'hide_count' => 0,
							'hide_paginator' => 0,
							'scrolling_paginator' => 0,
							'show_views_switcher' => $DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_views_switcher'],
							'listings_view_type' => $DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_views_switcher_default'],
							'listings_view_grid_columns' => $DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_views_switcher_grid_columns'],
							'grid_padding' => $DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_grid_padding'],
							...
							'2col_responsive' => $DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_listing_responsive_grid'],
							'masonry_layout' => $DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_grid_masonry_display'],
				...
				if (isset($_POST['order_by'])) {
					$_REQUEST['order_by'] = $shortcode_atts['order_by'] = directorypress_get_input_value($_POST, 'order_by', $shortcode_atts['order_by']);
					$_REQUEST['order'] = $shortcode_atts['order'] = directorypress_get_input_value($_POST, 'order', $shortcode_atts['order']);
				}
	...
	public function contact_form() {
		...
		if (!($type = $_REQUEST['type'])) {
			$error = esc_html__('The type of message required!', 'DIRECTORYPRESS');
		} else {
			check_ajax_referer('directorypress_' . $type . '_nonce', 'security');

			$validation = new directorypress_form_validation;
			if (!is_user_logged_in()) {
				$validation->set_rules('name', esc_html__('Contact name', 'DIRECTORYPRESS'), 'required');
				$validation->set_rules('email', esc_html__('Contact email', 'DIRECTORYPRESS'), 'required|valid_email');
			}
			$validation->set_rules('listing_id', esc_html__('Listing ID', 'DIRECTORYPRESS'), 'required');
			$validation->set_rules('message', esc_html__('Message', 'DIRECTORYPRESS'), 'required|max_length[1500]');
			if ($validation->run()) {
				$listing = new directorypress_listing();
				if ($listing->directorypress_init_lpost_listing($validation->result_array('listing_id'))) {
					...
					if (directorypress_recaptcha_validated()) {
						...
						$subject = sprintf(esc_html__('%s contacted you about your listing "%s"', 'DIRECTORYPRESS'), $name, $listing->title());
						...
						if (directorypress_mail($send_to_email, $subject, $body, $headers)) {
							...
							unset($_POST['email']);
							unset($_POST['message']);
							$success = esc_html__('You message was sent successfully!', 'DIRECTORYPRESS');
						} else {
			...
			if ($validation->run()) {
				...
				if ($listing->directorypress_init_lpost_listing($validation->result_array('listing_id'))) {
					...
					if (directorypress_recaptcha_validated()) {
						...
						$subject = sprintf(esc_html__('%s contacted you about "%s"', 'DIRECTORYPRESS'), $name, $listing->title());
	...
	public function keywords_search() {
		global $DIRECTORYPRESS_ADIMN_SETTINGS;
		$validation = new directorypress_form_validation;
		$validation->set_rules('term', esc_html__('Search term', 'DIRECTORYPRESS'));
		$validation->set_rules('directorytypes', esc_html__('Directorytypes IDs', 'DIRECTORYPRESS'));
		if ($validation->run()) {
