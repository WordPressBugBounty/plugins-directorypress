...
abstract class directorypress_widget extends WP_Widget {
	...
	public function __construct($id_base, $name, $description = '') {
		...
		$this->addField("textfield", "title", esc_html__("Title", "DIRECTORYPRESS"));
	...
	public function renderDependency($field){
		if ($field->dependency) {
			echo '<span class="directorypress-widget-dependency" data-dependency-element="'. esc_attr($field->dependency['element']) .'" data-dependency-value="'. esc_attr($field->dependency['value']) .'"></span>';
		}
	}
	...
	public function render_textfield_field($field, $instance) {
		...
		echo '<input class="widefat" id="'. esc_attr($id) .'" name="'. esc_attr($name) .'" type="text" value="'. esc_attr($value) .'" />';
	...
	public function render_hidden_field($field, $instance) {
		...
		echo '<input id="'. esc_attr($id) .'" name="'. esc_attr($name) .'" type="hidden" value="'. esc_attr($value) .'" />';
	}
	...
	public function render_dropdown_field($field, $instance) {
		...
		$this->renderDependency($field);
		$this->renderLabel($field, $id);
		echo '<select id="'. esc_attr($id) .'" name="'. esc_attr($name) .'" class="widefat directorypress-widget-select-control" data-original-name="'. esc_attr($field->name) .'">';
		foreach ($field->options as $label=>$option_val){
			...
			echo '<option value="'. esc_attr($option_val) .'"'. esc_attr($selected) .'>'. esc_html($label) .'</option>';
		}
		echo '</select>';
	...
	public function render_textarea_field($field, $instance) {
		...
		$this->renderDependency($field);
		$this->renderLabel($field, $id);
		echo '<textarea id="'. esc_attr($id) .'" name="'. esc_attr($name) .'" style="display:block;width:100%;">'. esc_html($value) .'</textarea>';
		$this->renderDescription($field, $id);
		echo '</p>';
	}
	...
	public function render_mapstyle_field($field, $instance) {
		...
		echo '<select id="'. esc_attr($id) .'" name="'. esc_attr($name) .'" class="widefat directorypress-widget-select-control">';
		echo '<option value="0" '. selected($value, 0) .'>'. esc_html__('Default', 'DIRECTORYPRESS') .'</option>';
		foreach (directorypress_map_styles() AS $style_name=>$style) {
			echo '<option value="'. esc_attr($style_name) .'" '. esc_attr(selected($value, $style_name)) .'>'. esc_html($style_name) .'</option>';
		}
		echo '</select>';
		$this->renderDescription($field, $id);
		echo '</p>';
	...
	public function render_ordering_field($field, $instance) {
		...
		//print_r($ordering);
		echo '<select id="'. esc_attr($id) .'" name="'. esc_attr($name) .'" class="widefat directorypress-widget-select-control">';
		echo '<option value="" '. esc_attr(selected($value, '', false)) .'>'. esc_html__('Default', 'DIRECTORYPRESS') .'</option>';
		foreach ($ordering AS $key=>$label) {
			//$value = $ordering_item['value'];
			//$label = $ordering_item['label'];
			
			echo '<option value="'. esc_attr($key) .'" '. esc_attr(selected($value, $key)) .'>'. esc_html($label) .'</option>';
	...
	public function render_package_field($field, $instance, $multiple = false) {
		...
		echo '<select id="'. esc_attr($name) . ($multiple ? '_select' : '') .'" name="'. esc_attr($name) . ($multiple ? '_select' : '') .'" '. ($multiple ? 'multiple="multiple" class="widefat directorypress-widget-select-multiple-control"' : 'class="widefat directorypress-widget-select-control"') .' data-original-name="'. esc_attr($field->name) .'">';
		echo '<option value="0" '. selected($value, 0) .'>'. esc_html__('- All packages -', 'DIRECTORYPRESS') .'</option>';
		foreach ($directorypress_object->packages->packages_array AS $package) {
			echo '<option value="'. esc_attr($package->id) .'" '. (in_array($package->id, explode(',', $value)) ? 'selected="selected"' : '') .'>'. esc_html($package->name) .'</option>';
		}
		echo '</select>';
		if ($multiple) {
	...
	public function render_directorytype_field($field, $instance, $multiple = false) {
		...
		if ($directorypress_object->directorytypes->isMultiDirectory()) {
			...
			$this->renderDependency($field);
			$this->renderLabel($field, $id);
			echo '<select id="'. esc_attr($name) . ($multiple ? '_select' : '') .'" name="'. esc_attr($name) . ($multiple ? '_select' : '') .'" '. ($multiple ? 'multiple="multiple" class="widefat directorypress-widget-select-multiple-control"' : 'class="widefat directorypress-widget-select-control"') .'>';
			echo '<option value="0" '. selected($value, 0) .'>'. esc_html__('- Auto -', 'DIRECTORYPRESS') .'</option>';
			foreach ($directorypress_object->directorytypes->directorypress_array_of_directorytypes AS $directorytype) {
				echo '<option value="'. esc_attr($directorytype->id) .'" '. (in_array($directorytype->id, explode(',', $value)) ? 'selected="selected"' : '') .'>'. esc_html($directorytype->name) .'</option>';
			}
			echo '</select>';
			if ($multiple) {
				echo '<input type="hidden" id="'. esc_attr($name) .'" name="'. esc_attr($name) .'" value="'. esc_attr($value) .'" />';
	...
	public function render_categoryfield_field($field, $instance, $multiple = false) {
		...
		$this->renderLabel($field, $id);
		echo '<select id="'. esc_attr($name) . ($multiple ? '_select' : '') .'" name="'. esc_attr($name) . ($multiple ? '_select' : '') .'" '. ($multiple ? 'multiple="multiple" class="widefat directorypress-widget-select-multiple-control"' : 'class="widefat directorypress-widget-select-control"') .'>';
		echo '<option value="" '. selected($value, '') .'>'. esc_html__('- No category selected -', 'DIRECTORYPRESS') .'</option>';
		ob_start();
		directorypress_renderOptionsTerms(DIRECTORYPRESS_CATEGORIES_TAX, 0, explode(',', $value));
		echo ob_get_clean(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped already
		echo '</select>';
		if ($multiple) {
			echo '<input type="hidden" id="'. esc_attr($name) .'" name="'. esc_attr($name) .'" value="'. esc_attr($value) .'" />';
		}
	...
	public function render_locationfield_field($field, $instance, $multiple = false) {
		...
		$this->renderLabel($field, $id);
		echo '<select id="'. esc_attr($name) . ($multiple ? '_select' : '') .'" name="'. esc_attr($name) . ($multiple ? '_select' : '') .'" '. ($multiple ? 'multiple="multiple" class="widefat directorypress-widget-select-multiple-control"' : 'class="widefat directorypress-widget-select-control"') .'>';
		echo '<option value="" '. selected($value, '') .'>'. esc_html__('- No location selected -', 'DIRECTORYPRESS') .'</option>';
		ob_start();
	...
	public function render_contentfields_field($field, $instance) {
		...
		$this->renderDependency($field);
		$this->renderLabel($field, $id);
		echo '<select id="'. esc_attr($name) .'_select" name="'. esc_attr($name) .'_select" multiple="multiple" class="widefat directorypress-widget-select-multiple-control">';
		echo '<option value="" '. esc_attr(selected($value, '')) .'>'. esc_html__('- All content fields -', 'DIRECTORYPRESS') .'</option>';
		echo '<option value="-1" '. selected($value, -1) .'>'. esc_html__('- No content fields -', 'DIRECTORYPRESS') .'</option>';
		foreach ($directorypress_object->search_fields->search_fields_array AS $search_field) {
			echo '<option value="'. esc_attr($search_field->field->id) .' '. (in_array($search_field->field->id, $fields_ids) ? 'selected' : '') .'>'. esc_html($search_field->field->name) .'</option>';
		}
		echo '</select>';
		echo '<input type="hidden" id="'. esc_attr($name) .'" name="'. esc_attr($name) .'" value="'. esc_attr($value) .'" />';
		$this->renderDescription($field, $id);
	...
	public function render_checkbox_field($field, $instance) {
		...
		if (!$field->options) {
			...
			echo '<input id="'. esc_attr($id) .'" name="'. esc_attr($name) .'" type="checkbox" value="1" '. esc_attr($checked) .'  data-original-name="'. esc_attr($field->name) .' />';
			$this->renderLabel($field, $id);
		} else {
			$this->renderLabel($field, $id);
			foreach ($field->options AS $option_name=>$key) {
				...
				echo '<br />';
				echo '<label>';
				echo '<input name="'. esc_attr($name) .'[]" type="checkbox" value="'. esc_attr($key) .'" '. esc_attr($checked) .' />';
