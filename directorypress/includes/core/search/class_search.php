	...
	public function display_search_button($on_row_search_button = false) {
		...
		echo '<div class="directorypress-search-form-button ' . esc_attr($classes) . '">';
			if ($this->args['search_button_type'] == 1 && !empty($icon)) {
				echo '<button type="submit" name="submit" class="btn btn-primary icon-left' . esc_attr($hide_button_class) . '" data-handler-hash="'. esc_attr($this->uid) .'">' . wp_kses_post($icon) . esc_html__('Search', 'DIRECTORYPRESS') . '</button>';
			} elseif ($this->args['search_button_type'] == 2  && !empty($icon)) {
				echo '<button type="submit" name="submit" class="btn btn-primary icon-right' . esc_attr($hide_button_class) . '" data-handler-hash="'. esc_attr($this->uid) .'">' . esc_html__('Search', 'DIRECTORYPRESS'). wp_kses_post($icon) . '</button>';
			}elseif ($this->args['search_button_type'] == 3) {
				echo '<button type="submit" name="submit" class="btn btn-primary' . esc_attr($hide_button_class) . '" data-handler-hash="'. esc_attr($this->uid) .'">' . esc_html__('Search', 'DIRECTORYPRESS') . '</button>';
