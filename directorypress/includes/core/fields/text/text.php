...
class directorypress_field_text extends directorypress_field {
	...
	public function configure($id, $action = '') {
		...
		if ($action == 'config') {
			...
			$validation->set_rules('max_length', esc_html__('Max length', 'DIRECTORYPRESS'), 'required|is_natural_no_zero');
			$validation->set_rules('regex', esc_html__('PHP RegEx template', 'DIRECTORYPRESS'));
			$validation->set_rules('is_phone', esc_html__('Is phone field', 'DIRECTORYPRESS'), 'is_checked');
			if ($validation->run()) {
	...
	public function validate_field_values(&$errors, $data) {
		...
		if (isset($_POST[$field_index]) && $_POST[$field_index] && $this->regex)
			if (@!preg_match('/^' . $this->regex . '$/', $_POST[$field_index]))
				$errors[] = sprintf(esc_html__("Field %s doesn't match template!", 'DIRECTORYPRESS'), $this->name);
	...
	public function validate_csv_values($value, &$errors) {
		if (!is_string($value))
			$errors[] = sprintf(esc_html__('Field %s must be a string!', 'DIRECTORYPRESS'), $this->name);
		elseif ($this->regex && @!preg_match('/^' . $this->regex . '$/', $value))
			$errors[] = sprintf(esc_html__("Field %s doesn't match template!", 'DIRECTORYPRESS'), $this->name);
		elseif (strlen($value) > $this->max_length)
			$errors[] = sprintf(esc_html__('The %s field can not exceed %s characters in length.', 'DIRECTORYPRESS'), $this->name, $this->max_length);
		else
			return $value;
	}
