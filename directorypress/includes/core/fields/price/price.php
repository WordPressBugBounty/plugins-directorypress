...
class directorypress_field_price extends directorypress_field {
	...
	public function is_field_not_empty($listing) {
		if ($this->data && (!empty($this->data['value']) || $this->data['option_selection'] == 'oncall')) {
			return true;
		} else {
			return false;
		}
	...
	public function configure($id, $action = '') {
		...
		if ($action == 'config') {
			...
			$validation->set_rules('currency_symbol', esc_html__('Currency symbol', 'DIRECTORYPRESS'), 'required');
			$validation->set_rules('decimal_separator', esc_html__('Decimal separator', 'DIRECTORYPRESS'), 'required|max_length[1]');
			$validation->set_rules('thousands_separator', esc_html__('Thousands separator', 'DIRECTORYPRESS'), 'max_length[1]');
			$validation->set_rules('symbol_position', esc_html__('Currency symbol position', 'DIRECTORYPRESS'), 'integer');
			$validation->set_rules('hide_decimals', esc_html__('Hide decimals', 'DIRECTORYPRESS'), 'required');
			$validation->set_rules('has_input_options', esc_html__('Has input options', 'DIRECTORYPRESS'), 'is_checked');
			$validation->set_rules('has_frontend_currency', esc_html__('Has Frontend Currency', 'DIRECTORYPRESS'), 'is_checked');
			$validation->set_rules('price_field_type', esc_html__('Price Field Type', 'DIRECTORYPRESS'), 'integer');
			$validation->set_rules('range_options[]', esc_html__('Range options', 'DIRECTORYPRESS'));
			if ($validation->run()) {
				if ( current_user_can( 'manage_options' ) ) {
					...
					if ($wpdb->update($wpdb->directorypress_fields, array('options' => serialize(
							...
							directorypress_add_notification(esc_html__('Field configuration was updated successfully!', 'DIRECTORYPRESS'));
					}
				}else{
	...
	/* public function validate_csv_values($value, &$errors) {
		if (!is_numeric($value)) {
			$errors[] = sprintf(esc_html__('The %s field must contain only numbers.', 'DIRECTORYPRESS'), $this->name);
		}
