...
function directorypress_get_term_parents($id, $tax, $link = false, $return_array = false, $separator = '/', &$chain = array()) {
	...
	if ($link && !is_wp_error($url)) {
		$chain[] = '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" href="' . esc_url($url) . '" title="' . esc_attr(sprintf(esc_html__('View all listings in %s', 'DIRECTORYPRESS'), $name)) . '"><span itemprop="name">' . esc_html($name) . '</span></a></li>';
	} else {
		$chain[] = $name;
	}
...
function directorypress_tax_dropdowns_menu_init($params) {
	...
	if ($terms) {
		...
		if ($exact_terms) {
			echo '<input type="hidden" id="exact_terms[' . esc_attr($uID) . ']" value="' . addslashes(implode(',', $exact_terms)) . '" />'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped already
		}
...
function _directorypress_tax_dropdowns_menu($tax, $parent = 0, $depth = 2, $current_level = 1, $term_id = null, $count = false, $exact_terms = array(), $hide_empty = false) {
	...
	if ($terms && ($depth == 0 || !is_numeric($depth) || $depth > $current_level)) {
		...
		if ($terms) {
			...
			$option_id_inc = uniqid();
				echo '<option id="' . esc_attr($term->slug) . esc_attr($option_id_inc) . '" value="' . esc_attr($term->term_id) . '" data-level="'. esc_attr($current_level) .'" data-parent="'. esc_attr($parent) .'" data-fonticolor="'. esc_attr($icon_color) .'" data-name="' . esc_attr($term->name)  . '" data-sublabel="' . esc_attr($sublabel) . '" ' . wp_kses_post($selected) . ' ' . wp_kses_post($icon) . ' data-icontype="' . esc_attr($icon_type) . '" ' . wp_kses_post($term_count) . '>' . esc_html($term->name) . '</option>';
...
function directorypress_tax_dropdowns_updateterms() {
	...
	if (!$title = directorypress_get_input_value($_POST, 'title'))
		$title = esc_html__('Select term', 'DIRECTORYPRESS');
...
function directorypress_renderOptionsTerms($tax, $parent, $selected_terms, $level = 0) {
	...
	foreach ($terms AS $term) {
		echo '<option value="' . esc_attr($term->term_id) . '" ' . (($selected_terms && (in_array($term->term_id, $selected_terms) || in_array($term->slug, $selected_terms))) ? 'selected' : '') . '>' . (str_repeat('&nbsp;&nbsp;&nbsp;', $level)) . esc_html($term->name) . '</option>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped already
...
function directorypress_termsSelectList($name, $tax = 'category', $selected_terms = array()) {
	...
	echo '<option value="" ' . ((!$selected_terms) ? 'selected' : '') . '>' . esc_html__('- Select All -', 'DIRECTORYPRESS') . '</option>';
...
function directorypress_terms_checklist($listing, $parent_allowed) {
	...
	echo '<select name="' . esc_attr(DIRECTORYPRESS_CATEGORIES_TAX) . '[]" multiple="multiple" class="directorypress-select2 category_selction">';
	foreach ($terms AS $term) {
		$checked = '';
		...
		echo _directorypress_terms_checklist($selected_categories, $term->term_id, $checked_categories_ids); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped already
	}
	echo '</select>';
}
...
function directorypress_tags_selectbox($post_id) {
	...
	echo '<select name="' . esc_attr(DIRECTORYPRESS_TAGS_TAX) . '[]" multiple="multiple" class="directorypress-token">';
	foreach ($terms AS $term) {
		$checked = '';
