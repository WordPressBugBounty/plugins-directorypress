...
class directorypress_listings_packages {
	...
	public function available_listings_descr($package_id, $action_string, $directorytype = false) {
		...
		if ($number) {
			$out = sprintf(esc_html__("You have <strong>%s</strong> free %s to %s in this package.", "DIRECTORYPRESS"), $number, _n($directorytype->single, $directorytype->plural, $number), $action_string);
		}
