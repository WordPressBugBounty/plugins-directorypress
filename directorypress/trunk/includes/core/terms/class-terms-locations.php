...
class DirectoryPress_Location_Terms extends DirectoryPress_Terms {
	...
	public function __construct($params) {
		...
		$this->params = $params;
		$this->view_all_terms = esc_html__("View all sublocations ->", "DIRECTORYPRESS");
				...
				//if ($this->depth == 0 || !is_numeric($this->depth) || $this->depth > $depth_level) {
					...
					foreach ($terms AS $term) {
							...
							if ($this->max_subterms != 0 && $counter > $this->max_subterms) {
								$html .= '<li><a href="' . get_term_link(intval($parent), DIRECTORYPRESS_LOCATIONS_TAX) . '">' . esc_html__('View All', 'DIRECTORYPRESS') . '</a></li>';
								break;
							} else
