				...
				if(!directorypress_is_listing_page()){
					if($listing->listing_view == 'show_grid_style'){
						if($field->is_hide_name_on_grid == 'show_only_label'){
							echo '<span class="directorypress-field-title" data-bs-toggle="tooltip" title="'.esc_url($field->value['url']).'" data-placement="auto">'. esc_html($field->name) .':</span>';
						}elseif($field->is_hide_name_on_grid == 'show_icon_label'){
							if ($field->icon_image){
								echo '<span class="directorypress-field-icon '. esc_attr($field->icon_image) .'" data-bs-toggle="tooltip" title="'.esc_url($field->value['url']).'" data-placement="auto"></span>';
							}
							echo '<span class="directorypress-field-title" data-bs-toggle="tooltip" title="'.esc_url($field->value['url']).'" data-placement="auto">'. esc_html($field->name) .':</span>';
