<script>
	...
	foreach ($fields AS $field): 
		...
		<?php } else { ?>
				fields_in_categories[<?php echo esc_attr($field->id); ?>] = [<?php echo implode(',', $field->categories); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped already ?>];
		<?php } ?>
	<?php endforeach; ?>
...
<div class="fields-dependency-alert alert alert-info"><?php esc_html_e('Fields can be depended on select category', 'DIRECTORYPRESS'); ?></div>
<?php
	foreach ($fields AS $field) {
		if (!$field->is_core_field){
			$field->renderInput();
