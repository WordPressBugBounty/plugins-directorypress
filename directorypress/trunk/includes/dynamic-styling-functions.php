...
if (!function_exists('directorypress_dynamic_css_injection')) {
    function directorypress_dynamic_css_injection(){
		...
		if(!empty($directorypress_styles)){
			echo '<script>';
			...
			echo 'php = {hasAdminbar: '.$is_admin_bar.',json: ('.$directorypress_json_encode.' != null) ? '.$directorypress_json_encode.' : "",styles:  \''.$clean_styles_w.'\'};var styleTag = document.createElement("style"),head = document.getElementsByTagName("head")[0];styleTag.type = "text/css";styleTag.innerHTML = php.styles;head.appendChild(styleTag);</script>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
...
<script>
	...
	var dynamic_styles = '<?php echo directorypress_clean_init_styles($directorypress_dynamic_styles_inject); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>';
