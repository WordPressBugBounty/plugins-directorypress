...
if (!function_exists('directorypress_renderMessages')) {
	function directorypress_renderMessages() {
		...
		foreach ($messages AS $type=>$messages) {
			...
			$message_class = 'alert alert-' . $type_class;

			echo '<div class="' . esc_attr($message_class) . '">';
			foreach ($messages AS $message) {
				echo '<div>' . trim(preg_replace("/<p>(.*?)<\/p>/", "$1", $message)) . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			echo '</div>';
		}
...
add_filter('directorypress_get_all_directory_pages', 'directorypress_el_directory_pages');
function directorypress_el_directory_pages($directory_pages) {
	global $wpdb;
	$directory_el_list = array('directorypress-main');
	
	if (!defined('ELEMENTOR_VERSION')) {
		return $directory_pages;
	}
	
	foreach ($directory_el_list AS $el) {
...
function directorypress_get_system_pages() {
	...
	$index_pages = apply_filters("directorypress_get_all_directory_pages", $index_pages);
