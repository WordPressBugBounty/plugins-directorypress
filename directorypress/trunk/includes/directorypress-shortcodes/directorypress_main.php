...
class directorypress_directory_handler extends directorypress_public {
	...
	public $archive_top_banner;
	public $archive_below_search_banner;
	public $archive_below_category_banner;
	public $archive_below_locations_banner;
	public $archive_below_listings_banner;
	public $listing_view;
	public $listing;
	public $category;
	...
	public function init($shortcode_atts = array(), $shortcode = 'directorypress-main') {
			...
			if (count($this->listings)) {
				...
				if ($DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_enable_breadcrumbs']) {
					if (!$DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_hide_home_link_breadcrumb'])
						$this->breadcrumbs[] = '<li><a href="' . directorypress_directorytype_url() . '">' . esc_html__('Home', 'DIRECTORYPRESS') . '</a></li>';
		...
		if ($DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_overwrite_page_title']) {
			...
			//add_filter( 'pre_wp_nav_menu', array($this, 'directorypress_remove_title_filter_nav_menu'), 10, 2 );
			
			// this filter fires after nav menu item creation is done
			add_filter( 'wp_nav_menu_items', array($this, 'directorypress_add_title_filter_non_menu'), 10, 2 );
	...
	public function directorypress_remove_title_filter_nav_menu( $nav_menu, $args ) {
		// we are working with menu, so remove the title filter
		remove_filter( 'the_title', array($this, 'setThemePageTitle'), 10, 2 );
		return $nav_menu;
	}


	public function directorypress_add_title_filter_non_menu( $items, $args ) {
