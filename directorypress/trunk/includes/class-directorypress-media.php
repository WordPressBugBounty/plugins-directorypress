...
class directorypress_media_handler {
	...
	public function fix_media_counts($views) {
		...
		$views['all'] = "<a href='upload.php'$class>" . sprintf(esc_html__('All <span class="count">(%s)</span>', 'DIRECTORYPRESS'), number_format_i18n($_total_posts)) . '</a>';
		...
		$views['detached'] = '<a href="upload.php?detached=1"' . ($detached ? ' class="current"' : '') . '>' . sprintf(esc_html__( 'Unattached <span class="count">(%s)</span>', 'DIRECTORYPRESS'), $total_orphans) . '</a>';
		return $views;
	}
	...
	public function upload_image() {
		...
		if (wp_verify_nonce($_GET['_wpnonce'], 'upload_images') && isset($_FILES['browse_file']) && ($_FILES['browse_file']['size'] > 0) && isset($_GET['post_id']) && ($post_id = esc_attr($_GET['post_id']))) {
				...
				if (directorypress_user_permission_to_edit_listing($post_id) || !$listing->post->post_author) {
					if (!$existed_images = get_post_meta($post_id, '_attached_image')){
						...
						if (in_array($uploaded_file_type, $allowed_file_types)) {
							...
							$result['error_msg'] = esc_html__('Please upload only image files (jpg, gif or png).', 'DIRECTORYPRESS');
						}
					
				}else{
					$result['error_msg'] = esc_html__('You do not have permissions to edit this listing!', 'DIRECTORYPRESS');
				}
			}else{
				$result['error_msg'] = esc_html__('Wrong listing ID.', 'DIRECTORYPRESS');
			}
				...
				if (directorypress_user_permission_to_edit_listing($post_id) || !$listing->post->post_author) {
					if (!$existed_images = get_post_meta($post_id, '_attached_image_clogo'))
						...
						if (in_array($uploaded_file_type, $allowed_file_types)) {
							...
							$result['error_msg'] = esc_html__('Please upload only image files (jpg, gif or png).', 'DIRECTORYPRESS');
					//} else
						//$result['error_msg'] = esc_html__('Can not upload images for this listing.', 'DIRECTORYPRESS');
				} else
					$result['error_msg'] = esc_html__('You do not have permissions to edit this listing!', 'DIRECTORYPRESS');
			} else{
				$result['error_msg'] = esc_html__('Wrong listing ID.', 'DIRECTORYPRESS');
			...
			$result['error_msg'] = esc_html__('Choose image to upload first!', 'DIRECTORYPRESS');
	...
	public function validate_attachments($package, &$errors) {
		if ($package->images_allowed || $package->videos_allowed) {
			...
			if ($package->images_allowed) {
				$validation->set_rules('attached_image_id[]', esc_html__('Listing images ID', 'DIRECTORYPRESS'));
				$validation->set_rules('attached_image_title[]', esc_html__('Listing images caption', 'DIRECTORYPRESS'));
				$validation->set_rules('attached_image_as_logo', esc_html__('Logo image selection', 'DIRECTORYPRESS'));
			}
			if ($package->videos_allowed) {
				$validation->set_rules('attached_video_id[]', esc_html__('Listing video ID', 'DIRECTORYPRESS'));
			}
