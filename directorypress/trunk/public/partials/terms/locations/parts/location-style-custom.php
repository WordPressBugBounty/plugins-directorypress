			...
			echo '<div class="directorypress-location-item">';
				...
				echo '<div class="directorypress-location-item-holder">';
					...
					echo '<div class="directorypress-parent-location">';
						echo '<a href="' . esc_url(get_term_link($term)) . '" title="' . esc_attr($term->name) . esc_attr($term_count) . '"><span class="loaction-name">' . esc_html($term->name) .'</span><span class="location-item-numbers">'. esc_html($term_count) .'</span></a>';
					echo '</div>';
				echo '</div>';
				if($instance->depth > 1 && $instance->has_child){
