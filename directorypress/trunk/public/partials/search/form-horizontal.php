<?php
	...
	$search_style = $DIRECTORYPRESS_ADIMN_SETTINGS['search-form-style'];
	...
	echo '<form action="'. esc_url($search_url) .'" class="search-form-style'. esc_attr($search_style) .' directorypress-search-layout-'. esc_attr($search_form->form_layout) .' directorypress-content-wrap directorypress-search-form" data-id="'. esc_attr($search_form->form_id) .'" id="directorypress-search-form-'. esc_attr($search_form->form_id) .'">';
			
			$search_form->display_hidden_fields();
		echo '<div class="directorypress-search-holder clearfix">';
			echo '<div class="search-container clearfix" style="margin-left:-'. esc_attr($search_form->args['gap_in_fields']) .'px; margin-right:-'. esc_attr($search_form->args['gap_in_fields']) .'px;">';
				if ($search_form->is_categories_or_Keywords_field()){
					...
					if ($search_form->is_categories() && $search_form->args['show_keywords_category_combo']) { 
						echo '<div class="keyword-search search-element-col pull-left directorypress-search-input-field-wrap" style="width:'. esc_attr($keyword_field_width) .'%; padding:0 '. esc_attr($search_form->args['gap_in_fields']) .'px;">';
							...
							directorypress_tax_dropdowns_menu_init($search_form->get_categories_dropmenu_params(esc_html__('Select category', 'DIRECTORYPRESS'), esc_html__('Enter Keyword', 'DIRECTORYPRESS')));
							if ($search_form->is_keyword_field_examples()){
								echo '<p class="directorypress-search-suggestions">';
									echo sprintf(esc_html__("Try to search: %s", "DIRECTORYPRESS"), wp_kses_post($search_form->get_keywords_examples()));
								echo '</p>';
							}
						echo '</div>';
						...
						echo '<div class="keyword-search search-element-col pull-left directorypress-search-input-field-wrap" style="width:'. esc_attr($keyword_field_width).'%; padding:0 '. esc_attr($search_form->args['gap_in_fields']).'px;">';
							...
							if ($search_form->is_keyword_field_examples()){
								echo '<p class="directorypress-search-suggestions">';
									printf(esc_html__("Try to search: %s", "DIRECTORYPRESS"), wp_kses_post($search_form->get_keywords_examples()));
								echo '</p>';
							}
						echo '</div>';
				...
				if ($search_form->is_locations_or_address_field()){
					...
					echo '<div class="address-search search-element-col pull-left directorypress-search-input-field-wrap" style="width:'. esc_attr($location_field_width) .'%; padding:0 '. esc_attr($search_form->args['gap_in_fields']).'px;">';
						if($search_form->is_default_fields_label()){
						...
						if ($search_form->is_locations()) {
							...
							echo '<div class="has-feedback">';
								...
								echo '<span class="directorypress-dropmenubox-button directorypress-form-control-feedback '. esc_attr($search_form->args['address_field_icon']) .'"></span>';
							echo '</div>';
						} 
					echo '</div>';
				...
				if ($search_form->is_radius()){
					if ($DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_miles_kilometers_in_search'] == 'miles'){
						$parameter = esc_html__('Mi', 'DIRECTORYPRESS');
						$parameter_full = esc_html__('Mile', 'DIRECTORYPRESS');
					}else{
						$parameter = esc_html__('Km', 'DIRECTORYPRESS');
						$parameter_full = esc_html__('Kilometer', 'DIRECTORYPRESS');
					}
					echo '<div class="radius-search-field cz-areaalider search-element-col pull-left" style="width:'. esc_attr($radius_field_width) .'%; padding:0 '. esc_attr($search_form->args['gap_in_fields']) .'px;">';
						if(!$DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_show_radius_tooltip'] && $search_form->is_default_fields_label()){
								...
								echo '<label>'.esc_html__('Search In Radius', 'DIRECTORYPRESS'). esc_html($search_form->get_radius_value()) . esc_html($parameter) .'</label>';
							}
				...
				if (!$search_form->args['on_row_search_button']){
					echo '<div class="search-element-col pull-right directorypress-search-submit-button-wrap" style="width:'. esc_attr($button_field_width) .'%; padding:0 '. esc_attr($search_form->args['gap_in_fields']) .'px; margin-top:'. esc_attr($search_form->args['search_button_margin_top']).'px;">';
						$search_form->display_search_button(true);
					echo '</div>';
				}
