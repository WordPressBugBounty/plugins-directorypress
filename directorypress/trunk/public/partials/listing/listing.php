<?php
	...
	if(!function_exists('directorypress_listing_grid_thumbnail')){
		function directorypress_listing_grid_thumbnail($listing){
			...
			echo '<a class="listing-thumbnail" href="'.esc_url(get_permalink()).'"><img alt="'. esc_attr($listing->title()) .'" src="'. esc_url(bfi_thumb($image_src, $param)).'" width="'. esc_attr($width) .'" height="'. esc_attr($height) .'" /></a>';
		}
	}
			...
			if($DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_ratings_addon']){
				if (!empty($rating)){
					...
					echo '<div class="listing-rating grid-rating">';
						...
						echo '&nbsp;<span class="review_rate-empty"><a class="simptip simptip-position-top simptip-movable" href="'. esc_url(get_permalink()) .'#comments-reviews" data-tooltip="'.esc_html__('Be first to rate', 'DIRECTORYPRESS').'">'.esc_html__('Rate Now', 'DIRECTORYPRESS').'</a></span>';
					echo '</div>';
				}
			}
		}
	...
	if(!function_exists('directorypress_listing_grid_bookmark')){
		function directorypress_listing_grid_bookmark($listing, $style, $in_favourites_icon = 'dicode-material-icons dicode-material-icons-heart', $not_in_favourites_icon = 'dicode-material-icons dicode-material-icons-heart'){
			...
			if (directorypress_bookmark_list($listing->post->ID)){
				...
				$link = '<a id="'.$listing->post->ID.'" href="javascript:void(0);" class="add_to_favourites btn" data-listingid="'. $listing->post->ID .'" data-bs-toggle="tooltip" title="'. esc_attr__('Remove Bookmarks', 'DIRECTORYPRESS').'" data-in_favourites_icon="'. esc_attr($in_favourites_icon) .'" data-not_in_favourites_icon="'. esc_attr($not_in_favourites_icon) .'">';
					$link .= '<span class="favourite-icon '. esc_attr($style_calss) .' checked '. esc_attr($in_favourites_icon) .'"></span>';
				$link .= '</a>';
				
			}else{
				$link = '<a id="'.$listing->post->ID.'" href="javascript:void(0);" class="add_to_favourites btn" data-listingid="'.$listing->post->ID.'" data-bs-toggle="tooltip" title="'. esc_attr__('Bookmark', 'DIRECTORYPRESS').'" data-in_favourites_icon="'. esc_attr($in_favourites_icon) .'" data-not_in_favourites_icon="'. esc_attr($not_in_favourites_icon) .'">';
	...
	if(!function_exists('directorypress_listing_grid_views')){
		function directorypress_listing_grid_views($listing){
			...
			echo '<p class="listing-views">'. sprintf(esc_html__('Views: %d', 'DIRECTORYPRESS'), (get_post_meta($listing->post->ID, '_total_clicks', true) ? esc_attr(get_post_meta($listing->post->ID, '_total_clicks', true)) : 0)).'</p>';
		}
	}
	...
	if($listing_style_to_show == 'show_grid_style'){
			if ($directorypress_object->directorypress_get_property_of_shortcode('directorypress-main', 'is_favourites') && directorypress_bookmark_list($listing->post->ID)){
				echo '<div class="directorypress-remove-from-favourites-list" data-listingid="'. esc_attr($listing->post->ID) .'" title="'.esc_attr(esc_html__('Remove from favourites list', 'DIRECTORYPRESS')).'"></div>';
			}
